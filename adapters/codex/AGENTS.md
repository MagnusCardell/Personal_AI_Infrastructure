...
That launcher may run `doctor`, `exec-proof`, `workloop-once`, `audit-run`, `task-run`, and `audit-task` for bounded adapter validation only.
Runtime proof, work-loop, and task runs must capture Codex JSONL event streams for event-attributed write-boundary validation.
S15C task execution may use workspace-write only inside `PAI_DIR/adapters/codex/runs/s15c/workspace/`.
